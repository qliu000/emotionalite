...
function draw() {
  ...
  // 🔊 Mic 开启时，用声音驱动两个 slider
  if (micEnabled && mic) {
    const level = mic.getLevel(); // 0 ~ 大概 0.3
    const mappedVol = constrain(map(level, 0, 0.25, 0, 1), 0, 1);
    const mappedPit = mappedVol;  // 这里简单用同一个数值做 pitch
